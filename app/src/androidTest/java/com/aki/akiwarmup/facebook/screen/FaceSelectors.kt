...
/**
 * Tập hợp các chuỗi nhãn văn bản (text labels) thường gặp trên giao diện Facebook hoặc TikTok
 * được dùng làm điều kiện định vị phần tử UI bằng text.
 */
object FaceText {
...
/**
 * Tập hợp các ID tài nguyên giao diện (Android Resource ID) của ứng dụng TikTok,
 * được dùng làm điều kiện định vị các phần tử UI đặc thù bằng ID.
 */
object TiktokId {
...
/**
 * Tập hợp các chuỗi mô tả thuộc tính `contentDescription` của các phần tử giao diện
 * dùng trong việc tìm kiếm các nút/icon không chứa text.
 */
object TiktokDesc {
